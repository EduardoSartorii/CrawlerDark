...
@register_connector
class DeepWebConnector(SimpleConnector):
    """Deep web forum monitoring."""

    name: ClassVar[str] = "deepweb"
    category_group: ClassVar[str] = "darkweb"
    default_category: ClassVar[FindingCategory] = FindingCategory.DEEP_WEB
    default_source: ClassVar[str] = "deepweb"
    opsec_profile_name: ClassVar[str] = "darkweb"

    def _demo_samples(self) -> list[dict[str, Any]]:
        return [
            {
                "title": "[deepweb] Deep web forum monitoring",
                "description": (
                    "Deep web forum monitoring. Sample IOCs for pipeline validation: "
                    "user@corp-demo.com 198.51.100.42 malware.example "
                    "CVE-2024-12345 wallet 1A1zP1eP5QGefi2DMPTfTL5SLmv7DivfNa"
                ),
                "url": "https://example.local/deepweb/item/1",
                "author": "deepweb-collector",
                "tags": ["darkweb", "deepweb", "demo"],
                "category": "deep_web",
                "severity": "medium",
            }
        ]
