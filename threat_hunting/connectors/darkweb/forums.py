...
@register_connector
class ForumsConnector(SimpleConnector):
    """Underground forum monitoring."""

    name: ClassVar[str] = "forums"
    category_group: ClassVar[str] = "darkweb"
    default_category: ClassVar[FindingCategory] = FindingCategory.DARK_WEB
    default_source: ClassVar[str] = "forums"
    opsec_profile_name: ClassVar[str] = "darkweb"

    def _demo_samples(self) -> list[dict[str, Any]]:
        return [
            {
                "title": "[forums] Underground forum monitoring",
                "description": (
                    "Underground forum monitoring. Sample IOCs for pipeline validation: "
                    "user@corp-demo.com 198.51.100.42 malware.example "
                    "CVE-2024-12345 wallet 1A1zP1eP5QGefi2DMPTfTL5SLmv7DivfNa"
                ),
                "url": "https://example.local/forums/item/1",
                "author": "forums-collector",
                "tags": ["darkweb", "forums", "demo"],
                "category": "dark_web",
                "severity": "medium",
            }
        ]
