...
Responsibility
--------------
Telegram channel/group monitoring for leaks and IOCs
...
@register_connector
class TelegramConnector(SimpleConnector):
    """Telegram channel/group monitoring for leaks and IOCs."""

    name: ClassVar[str] = "telegram"
    category_group: ClassVar[str] = "social"
    default_category: ClassVar[FindingCategory] = FindingCategory.DARK_WEB
    default_source: ClassVar[str] = "telegram"
    opsec_profile_name: ClassVar[str] = "social"

    def _demo_samples(self) -> list[dict[str, Any]]:
        return [
            {
                "title": "[telegram] Telegram channel/group monitoring for leaks and IOCs",
                "description": (
                    "Telegram channel/group monitoring for leaks and IOCs. Sample IOCs for pipeline validation: "
                    "user@corp-demo.com 198.51.100.42 malware.example "
                    "CVE-2024-12345 wallet 1A1zP1eP5QGefi2DMPTfTL5SLmv7DivfNa"
                ),
                "url": "https://example.local/telegram/item/1",
                "author": "telegram-collector",
                "tags": ["social", "telegram", "demo"],
                "category": "dark_web",
                "severity": "medium",
            }
        ]
