...
Responsibility
--------------
Censys host/certificate discovery
...
@register_connector
class CensysConnector(SimpleConnector):
    """Censys host/certificate discovery."""
    ...
    default_source: ClassVar[str] = "censys"
    opsec_profile_name: ClassVar[str] = "default"

    def _demo_samples(self) -> list[dict[str, Any]]:
        return [
            {
                "title": "[censys] Censys host/certificate discovery",
                "description": (
                    "Censys host/certificate discovery. Sample IOCs for pipeline validation: "
                    "user@corp-demo.com 198.51.100.42 malware.example "
                    "CVE-2024-12345 wallet 1A1zP1eP5QGefi2DMPTfTL5SLmv7DivfNa"
                ),
                "url": "https://example.local/censys/item/1",
                "author": "censys-collector",
                "tags": ["threat_intel", "censys", "demo"],
