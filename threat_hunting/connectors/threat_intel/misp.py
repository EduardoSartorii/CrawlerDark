...
Responsibility
--------------
MISP event/attribute collection via PyMISP
...
@register_connector
class MispConnector(SimpleConnector):
    """MISP event/attribute collection via PyMISP."""
    ...
    default_source: ClassVar[str] = "misp"
    opsec_profile_name: ClassVar[str] = "default"

    def _demo_samples(self) -> list[dict[str, Any]]:
        return [
            {
                "title": "[misp] MISP event/attribute collection via PyMISP",
                "description": (
                    "MISP event/attribute collection via PyMISP. Sample IOCs for pipeline validation: "
                    "user@corp-demo.com 198.51.100.42 malware.example "
                    "CVE-2024-12345 wallet 1A1zP1eP5QGefi2DMPTfTL5SLmv7DivfNa"
                ),
                "url": "https://example.local/misp/item/1",
                "author": "misp-collector",
                "tags": ["threat_intel", "misp", "demo"],
                "category": "ioc",
                "severity": "medium",
            }
        ]
