...
@register_connector
class ThreatFoxConnector(SimpleConnector):
    """ThreatFox IOC feed."""
    ...
    default_source: ClassVar[str] = "threatfox"
    opsec_profile_name: ClassVar[str] = "default"

    def _demo_samples(self) -> list[dict[str, Any]]:
        return [
            {
                "title": "[threatfox] ThreatFox IOC feed",
                "description": (
                    "ThreatFox IOC feed. Sample IOCs for pipeline validation: "
                    "user@corp-demo.com 198.51.100.42 malware.example "
                    "CVE-2024-12345 wallet 1A1zP1eP5QGefi2DMPTfTL5SLmv7DivfNa"
                ),
                "url": "https://example.local/threatfox/item/1",
                "author": "threatfox-collector",
                "tags": ["threat_intel", "threatfox", "demo"],
                "category": "ioc",
                "severity": "medium",
            }
        ]
