...
@register_connector
class UrlhausConnector(SimpleConnector):
    """URLhaus malware URL feed."""

    name: ClassVar[str] = "urlhaus"
    category_group: ClassVar[str] = "threat_intel"
    default_category: ClassVar[FindingCategory] = FindingCategory.MALWARE
    default_source: ClassVar[str] = "urlhaus"
    opsec_profile_name: ClassVar[str] = "default"

    def _demo_samples(self) -> list[dict[str, Any]]:
        return [
            {
                "title": "[urlhaus] URLhaus malware URL feed",
                "description": (
                    "URLhaus malware URL feed. Sample IOCs for pipeline validation: "
                    "user@corp-demo.com 198.51.100.42 malware.example "
                    "CVE-2024-12345 wallet 1A1zP1eP5QGefi2DMPTfTL5SLmv7DivfNa"
                ),
                "url": "https://example.local/urlhaus/item/1",
                "author": "urlhaus-collector",
                "tags": ["threat_intel", "urlhaus", "demo"],
                "category": "malware",
                "severity": "medium",
            }
        ]
