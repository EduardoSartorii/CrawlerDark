"""alienvault_otx connector.

Responsibility
--------------
AlienVault OTX pulse collection
...
@register_connector
class AlienVaultOtxConnector(SimpleConnector):
    """AlienVault OTX pulse collection."""

    name: ClassVar[str] = "alienvault_otx"
    category_group: ClassVar[str] = "threat_intel"
    default_category: ClassVar[FindingCategory] = FindingCategory.IOC
    default_source: ClassVar[str] = "alienvault_otx"
    opsec_profile_name: ClassVar[str] = "default"

    def _demo_samples(self) -> list[dict[str, Any]]:
        return [
            {
                "title": "[alienvault_otx] AlienVault OTX pulse collection",
                "description": (
                    "AlienVault OTX pulse collection. Sample IOCs for pipeline validation: "
                    "user@corp-demo.com 198.51.100.42 malware.example "
                    "CVE-2024-12345 wallet 1A1zP1eP5QGefi2DMPTfTL5SLmv7DivfNa"
                ),
                "url": "https://example.local/alienvault_otx/item/1",
                "author": "alienvault_otx-collector",
                "tags": ["threat_intel", "alienvault_otx", "demo"],
                "category": "ioc",
                "severity": "medium",
            }
        ]
