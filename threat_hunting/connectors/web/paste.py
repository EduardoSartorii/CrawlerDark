...
Responsibility
--------------
Paste site hunting for credentials and leaks
...
@register_connector
class PasteConnector(SimpleConnector):
    """Paste site hunting for credentials and leaks."""

    name: ClassVar[str] = "paste"
    category_group: ClassVar[str] = "web"
    default_category: ClassVar[FindingCategory] = FindingCategory.LEAK
    default_source: ClassVar[str] = "paste"
    opsec_profile_name: ClassVar[str] = "default"

    def _demo_samples(self) -> list[dict[str, Any]]:
        return [
            {
                "title": "[paste] Paste site hunting for credentials and leaks",
                "description": (
                    "Paste site hunting for credentials and leaks. Sample IOCs for pipeline validation: "
                    "user@corp-demo.com 198.51.100.42 malware.example "
                    "CVE-2024-12345 wallet 1A1zP1eP5QGefi2DMPTfTL5SLmv7DivfNa"
                ),
                "url": "https://example.local/paste/item/1",
                "author": "paste-collector",
                "tags": ["web", "paste", "demo"],
                "category": "leak",
                "severity": "medium",
            }
