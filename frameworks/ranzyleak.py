...
class VigilantOnion:
    def __init__(self,save_path_log:str, debug:bool, webdriver_path:str):
        ...
        self.onion_urls = "http://37rckgo66iydpvgpwve7b2el5q2zhjw4tv4lmyewufnpx4lhkekxkoqd.onion"
    ...
    def parser_html(self, content:str,url:str) -> None:
        try:
            time.sleep(15)
            feed = content.find('div', {"class" : "container py-5"})

            for post in feed.findAll('div', {"class":"article mb-5"}):
                name_enterprise = post.find("h3", {'class': 'mb-3'}).getText()
                url_post = url
    ...
    def parser_event(self,name_enterprise:str,url_enterprise:str,content:str) -> None:
        try:
            time.sleep(5)
            name_file = f"utils/screenshot/{datetime.today().strftime('%Y-%m-%d-%H-%M-%S-%f')}-ranzyleak.png"
            if self.validateDB(data=url_enterprise,enterprise="onion") is not False:
                self.driver.get_screenshot_as_file(name_file)
                self.feedmisp.start(date_post= datetime.today().strftime("%Y-%m-%d"),enterprise= name_enterprise, url=url_enterprise, description= "Ranzyleak",photo=name_file)
            else:
