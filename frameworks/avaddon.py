...
class VigilantOnion:
    def __init__(self,save_path_log:str, debug:bool, webdriver_path:str):
        ...
        self.onion_urls = "http://avaddongun7rngel.onion/disclosed"
    ...
    def parser_html(self, content:str,url:str) -> None:
        try:
            time.sleep(15)
            feed = content.find('div', {"class" : "col-md-6"})

            for post in feed.findAll('div', {"class":"border-top"}):
                name_enterprise = post.find("h3").getText()
                url_post = post.find_all('a')
                self.driver.get(url_post[1].get('href'))
                self.parser_event(name_enterprise=name_enterprise ,url_enterprise=url_post[0].get('href'),content= bs(self.driver.page_source, "html.parser"))
        except Exception as error:
    ...
    def parser_event(self,name_enterprise:str,url_enterprise:str,content:str) -> None:
        try:
            ...
            name_file = f"utils/screenshot/{datetime.today().strftime('%Y-%m-%d-%H-%M-%S-%f')}-avaddon.png"
            if self.validateDB(data=url_enterprise,enterprise="onion") is not False:
                self.driver.get_screenshot_as_file(name_file)
                self.feedmisp.start(date_post= datetime.today().strftime("%Y-%m-%d"),enterprise= name_enterprise, url=url_enterprise, description= "Avaddon",photo=name_file)
            else:
