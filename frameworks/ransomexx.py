...
class VigilantOnion:
    def __init__(self,save_path_log:str, debug:bool, webdriver_path:str):
        ...
        self.onion_urls = "http://rnsm777cdsjrsdlbs4v5qoeppu3px6sb2igmh53jzrx7ipcrbjz5b2ad.onion"
    ...
    def parser_html(self, content:str,url:str) -> None:
        try:
            time.sleep(15)
            feed = content.find('div', {"class" : "container py-4"})

            for post in feed.findAll('div', {"class":"justify-content-md-center"}):
                name_enterprise = post.find("h5", {'class': 'card-title'}).getText()
                url_post = post.find('a', {'class': "btn-outline-primary"})
                self.driver.get(f'{url}{url_post["href"]}')
                self.parser_event(name_enterprise=name_enterprise ,url_enterprise=url_post["href"],content= bs(self.driver.page_source, "html.parser"))
        except Exception as error:
    ...
    def validateDB(self, data:str, enterprise:str) -> bool:
        if self.database.compare(data=data,enterprise=enterprise):
            ...
            if self.database.save(data=data, enterprise=enterprise):
                ...
                return False    
    def parser_event(self,name_enterprise:str,url_enterprise:str,content:str) -> None:
        try:
            time.sleep(5)
            name_file = f"utils/screenshot/{datetime.today().strftime('%Y-%m-%d-%H-%M-%S-%f')}-ransomexx.png"
            if self.validateDB(data=url_enterprise,enterprise="onion") is not False:
                self.driver.get_screenshot_as_file(name_file)
                self.feedmisp.start(date_post= datetime.today().strftime("%Y-%m-%d"),enterprise= name_enterprise, url=url_enterprise, description= "Ransomexx",photo=name_file)
            else:
